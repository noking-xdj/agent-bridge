...
| 工具 | 说明 | 注解 |
|------|------|------|
| `codex_delegate` | 委托任务给 Codex（写代码、执行命令等） | 可修改、有外部交互 |
| `codex_ask` | 向 Codex 提问并获取回答 | 有外部交互 |
| `codex_collaborate` | 双向协作 — 两个 Agent 轮流贡献 | 可修改、有外部交互 |
| `codex_exec` | 让 Codex 执行 shell 命令 | 可修改、有外部交互 |
| `codex_review` | 让 Codex 进行代码审查 | 有外部交互 |
| `codex_status` | 查看委托任务的状态 | 只读、可重复调用 |
| `shared_context_write` | 写入共享上下文存储 | 可修改（覆盖）、可重复调用 |
| `shared_context_read` | 读取共享上下文存储 | 只读、可重复调用 |
| `shared_context_delete` | 删除共享上下文条目 | 可修改、可重复调用 |
...
- 防重入保护（Single-flight guard）防止并发初始化竞争
